...
export default function Deployed({ tasks, handleDelete, handleUpdate }) {
    return (
        <div>
            <div className="bg-white h-[340px] rounded-2xl overflow-y-scroll scrollbar-hide">
                ...
                <div>
                    {
                        tasks?.deployed?.data?.map(item => <div title={`From ${item?.start_date} To ${item?.end_date}`} key={item?._id} className="p-2">
